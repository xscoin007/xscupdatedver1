...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #XSCOINTEST2\r");
            Send(hSocket, "WHO #XSCOINTEST2\r");
        } else {
            // randomly join #XSCOIN00-#XSCOIN05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #XSCOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #XSCOIN%02d\r", channel_number).c_str());
        }
